...
const Hyperlink = forwardRef(
  (
    ...
    if (target === "_blank") {
      if (targetBlankRel) {
        appliedRel = `${rel} ${targetBlankRel}`;
      } else {
        appliedRel = `${rel} noopener noreferrer`;
      }
    } else {
      appliedRel = rel;
    }
    ...
    const defaultClasses =
      "flex items-center text-brand-default decoration-inherit hover:text-brand-strong hover:underline text-base focus-visible:rounded-sm focus-visible:outline focus-visible:outline-2 focus-visible:outline-brand-strong";
...
export const HyperlinkProps = {
   ...
   * @default ''
   */
  targetBlankRel: PropTypes.string,
  /**
...
Hyperlink.propTypes = HyperlinkProps;
