...
import {
  INPUT_BADGE_COLOUR_CLASSES,
  MENTION_INPUT_STYLES,
} from "./constants/inputWithVariables";
import { getMentionInputClassName } from "./utils";
 ...
 */
const InputWithVariables = forwardRef(
  (
    {
      a11ySuggestionsListLabel = "Variable dataset list",
    ...
    <div>
      {(label || cornerHintText) && (
        <div className="mb-1 flex" role={role}>
          <label
            htmlFor={id}
            id={`${id}label-wrap`}
      ...
      <MentionsInput
        ...
        style={MENTION_INPUT_STYLES}
        classNames={getMentionInputClassName({
          readonly,
          disabled,
          wrapperClassName,
          errorText,
      ...
      >
        {variables.map(
          ({
            ...
            const re = forceSpaceBeforeTrigger
              ? trigger
              : new RegExp(
                  `(\\${trigger}([^${
                    allowSpaceInQuery ? "" : "\\s"
            ...
            return (
              <Mention
                ...
                displayTransform={(ids, display) =>
                  ids === newDatasetId ? "" : `  ${display}  `
                }
                markup={markup ?? `${trigger}[__display__](__id__)`}
                trigger={re}
                data={data}
                onAdd={onVariableAdd}
                ...
                className={twClassNames(
                  ...
                  INPUT_BADGE_COLOUR_CLASSES[badgeType]?.BG_TEXT || ""
                )}
              />
            );
          }
      ...
      {errorText && (
        <p className="text-sm text-danger-default" id={`${id}error-wrap`}>
          {errorText}
        </p>
      )}
      {description && (
        <p
          className={twClassNames("text-sm text-neutral-weaker", {
            "mt-2 ": errorText,
          })}
        >
          {description}
...
InputWithVariables.propTypes = {
  /** This label would be exposed to screen readers when variables popup appears */
  a11ySuggestionsListLabel: PropTypes.string,
  /** Keep variables popover open even if the user separates keywords with spaces. */
  allowSpaceInQuery: PropTypes.bool,
  /** Renders the variables popover List above the cursor if there is not enough space below */
  allowSuggestionsAboveCursor: PropTypes.bool,
  /** It provides the hint text to user. */
  cornerHintText: PropTypes.string,
  /** The description of the input field. */
  description: PropTypes.string,
  /** Boolean option that determines whether the InputField is disabled or not. */
  ...
  /** Field is mandatory or not. If true, then "*" symbol will be added. */
  isMandatory: PropTypes.bool,
  /** The value of input label. */
  label: PropTypes.string,
  /** Callback fired when the input element loses focus.Passes true as second argument if the blur was caused by a mousedown on a suggestion.args: event, clickedSuggestion */
  onBlur: PropTypes.func,
  /** Callback fired when the value of input element is changed.args: event, newValue, newPlainTextValue, mention */
  onChange: PropTypes.func,
  /** Callback fired when the input field is in the focus. */
  ...
  /** Set role for input field. */
  role: PropTypes.string,
  /** The value containing markup for variable */
  value: PropTypes.oneOfType([PropTypes.string, PropTypes.number]).isRequired,
  /** Array of variable props */
  variables: PropTypes.arrayOf(
    PropTypes.shape({
      /** Append a space when a variable has been added */
      ...
      /** Callback invoked when a variable has been added, args : id, display, startPos, endPos */
      onVariableAdd: PropTypes.func,
      /** Defines the char upon which to trigger querying the data source */
      trigger: PropTypes.string,
    })
  ).isRequired,
  /** A custom CSS class name to style the input field. */
