...
import {
  STATS_DEC,
  STATS_INC,
  STATS_OPTION_CLASSES,
  STATS_VARIANTS,
  STATS_WRAPPER_CLASSES,
...
const Stats = ({
  ...
  option,
  textColor = "text-neutral-default",
  variant = STATS_VARIANTS.SIMPLE,
}) => {
  const onKeyDown = e => {
    if (e.key === KEYBOARD_KEYS.ENTER) option.onClick?.(e);
    if (e.key === KEYBOARD_KEYS.SPACE) option.onClick?.(e);
  };
  ...
  const isClickable = typeof option.onClick === "function";

  return (
    <div
      key={option.id}
      className={twClassNames(
        STATS_WRAPPER_CLASSES[variant],
        {
          "pb-10": variant === STATS_VARIANTS.GRAPH_VARIANT && option.link,
          "shadow-sm": !hideBoxShadow,
          "border border-neutral-default": hasBorder,
          "cursor-pointer": isClickable,
        },
        "relative overflow-hidden rounded-lg bg-neutral-default px-4 focus-visible:z-10 focus-visible:outline-brand-strong sm:px-6",
        cardWrapperClassname
      )}
      {...(isClickable && {
        onClick,
        ...
        role: "button",
        tabIndex: 0,
        "aria-label": `${option.name}: ${option.stat} ${option.change} ${option.changeType}`,
      })}
      {...(id !== null && { id })}
      ...
      <dl>
        ...
        >
          {variant === STATS_VARIANTS.WITH_ICON && (
            <div
              className={twClassNames(
                "absolute rounded-md bg-brand-default p-3",
...
Stats.propTypes = {
  ...
  option: PropTypes.shape({
    /** Text describing the change in the statistic (e.g., "+5.2%", "-10"). */
    change: PropTypes.string,
    /** Indicates the nature of the change ('increase' or 'decrease'), used for styling. Use `STATS_INC` or `STATS_DEC`. */
    changeType: PropTypes.oneOf([STATS_INC, STATS_DEC]),
    /** A React node (e.g., a small chart) to display within the card (used with `graph_variant`). */
    graph: PropTypes.node,
    /** A React node (usually an icon) to display (used with `with_icon` variant). */
    icon: PropTypes.node,
    /** Custom CSS class name for the container wrapping the `icon` (used with `with_icon` variant). */
    iconContainerWrapperClass: PropTypes.string,
    /** Unique identifier for this specific stat option. */
    id: PropTypes.oneOfType([PropTypes.number, PropTypes.string]),
    /** A React node (e.g., `<a>` or `Link`) to render as a link at the bottom. */
    link: PropTypes.node,
    /** Text content for the `link`. */
    linkText: PropTypes.string,
    /** A React node (e.g., a dropdown menu) to display in the top-right (used with `graph_variant`). */
    menuDropdown: PropTypes.node,
    /** The title or label for the statistic. */
    name: PropTypes.oneOfType([PropTypes.node, PropTypes.string]),
    /** Callback function executed when the card is clicked. Makes the card interactive. */
    onClick: PropTypes.func,
    /** The main statistical value to display. */
    stat: PropTypes.oneOfType([PropTypes.node, PropTypes.string]),
    /** Additional text displayed below the main `stat` value. */
  ...
  variant: PropTypes.oneOf(Object.values(STATS_VARIANTS)),
};
